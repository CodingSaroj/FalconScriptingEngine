...
namespace Falcon
{
    namespace Assembler
    {
        ...
        Token Lexer::processNumber(bool sign)
        {
            ...
            if (m_CurrentChar == '0')
            {
                if (this->advance() == 'x' || this->advance() == 'X')
                {
                    base = HEX;
                }
                else
            ...
            while (m_CurrentChar != '\0' && whiteSpaces.find(m_CurrentChar) == std::string::npos)
            {
                ...
                {
                    if (octDigits.find(m_CurrentChar) == std::string::npos)
                    {
                        break;
                    }
                }
                ...
                {
                    if (decDigits.find(m_CurrentChar) == std::string::npos)
                    {
                        break;
                    }
                }
                ...
                {
                    if (hexDigits.find(m_CurrentChar) == std::string::npos)
                    {
                        break;
                    }
                }
            ...
            if (isFloat)
            {
                return Token(TokenType::FLOAT, std::strtod(number.c_str(), NULL));
            }

            if (base == OCT)
            {
                return Token(TokenType::UINT, std::strtoul(number.c_str(), NULL, 8));
            }
            else if (base == DEC)
            {
                if (sign)
                {
                    return Token(TokenType::INT, std::strtol(number.c_str(), NULL, 10));
                }
                else
                {
                    return Token(TokenType::UINT, std::strtoul(number.c_str(), NULL, 10));
                }
            }
            else
            {
                return Token(TokenType::UINT, std::strtoul(number.c_str(), NULL, 16));
            }
        }
        ...
        Token Lexer::processStr()
        {
            ...
            while (m_CurrentChar != '\0' && whiteSpaces.find(m_CurrentChar) == std::string::npos)
            {
                if (identChars.find(m_CurrentChar) == std::string::npos && decDigits.find(m_CurrentChar) == std::string::npos)
                {
                    break;
                }
            ...
            if (std::find(RegisterType::names.begin(), RegisterType::names.end(), toUpper(str)) != RegisterType::names.end())
            {
                return Token(TokenType::REGISTER, toUpper(str));
            }
            else if (std::find(OpCode::names.begin(), OpCode::names.end(), toUpper(str)) != OpCode::names.end())
            {
